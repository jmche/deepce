...
class DrugGeneAttention(nn.Module):
    ...
    def forward(self, trg, enc_src, trg_mask, src_mask):
        ...
        for layer in self.layers:
            # print('i:' + str(i))
            # i=i+1
            # print('trg: ' + str(trg.device) + ' ' + str(trg.shape))
            # print('enc_src: ' + str(enc_src.device) + ' ' + str(enc_src.shape))
            # if(not trg_mask is None):
            #     print('trg_mask: ' + str(trg_mask.device) + ' ' + str(trg_mask.shape))
            #     print(trg_mask.shape)
            # if(not src_mask is None):
            #     print('src_mask: ' + str(src_mask.device) + ' ' + str(src_mask.shape))
