...
def pearsonJmche(x, y):
    ...
    r_den = torch.norm(xm, 2, dim=-1) * torch.norm(ym, 2, dim=-1)
    r_val = r_num / r_den
    return torch.sum(1-r_val)

def cosineSimJmche(x, y):
    return(torch.sum(1-F.cosine_similarity(x,y)))

def pearsonNcosine(x, y):
    mean_x = torch.mean(x, dim=-1, keepdim=True)
    mean_y = torch.mean(y, dim=-1, keepdim=True)
    xm = x.sub(mean_x)
    ym = y.sub(mean_y)
    ...
    r_den = torch.norm(xm, 2, dim=-1) * torch.norm(ym, 2, dim=-1)
    r_val = r_num / r_den
    return(torch.sum(1-F.cosine_similarity(x,y)) + torch.sum(1-r_val))
