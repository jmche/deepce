...
class Molecule(object):
    ...
    def read_from_smiles(self, smiles):
        ...
        #아래 3개 line code추가
        standardized_mol = MolStandardize.Standardizer().standardize(mol)
        canonical_smiles = rdkit.Chem.MolToSmiles(standardized_mol, canonical=True, isomericSmiles = False)
        mol = rdkit.Chem.MolFromSmiles(canonical_smiles)
        if not mol:
